...
function ResultPage() {
  ...
  const roles = interests.map((item) => BODY_PART_INFO[item]?.role).filter((item): item is string => !!item);
  const foods = unique(interests.flatMap((item) => FOOD_ROWS[item] ?? []).map((row) => JSON.stringify(row))).map((row) => JSON.parse(row) as FoodRow);
  const lifestyle = unique([...conditions.flatMap((item) => DISEASE_INFO[item]?.lifestyle ?? []), ...BASE_LIFESTYLE]);
  const symptoms = unique(interests.flatMap((item) => BODY_PART_INFO[item]?.symptoms ?? []));
    ...
    <main className="min-h-screen bg-[#ffffff]">
      ...
      <div className="mx-auto max-w-[960px] px-5 py-10 sm:px-8 sm:py-14">
        ...
        <Section index={2} title="신체정보 분석">
          <p className="text-lg leading-relaxed text-[#6b7280]">BMI는 키와 몸무게로 체격을 가늠하는 지표입니다. 계산식은 체중(kg) ÷ 키(m)²입니다.</p>
        ...
        <Section index={9} title="비의료정보 안내">
          <p className="text-lg leading-relaxed text-[#6b7280]">이 내용은 AI가 생성한 일반 건강정보이며 진단·치료를 대신하지 않습니다. 건강 상태에 대한 판단과 조치는 의료 전문가와 상담해 주세요.</p>
        </Section>
